...
def plot_total_sales_by_store(df, unit="day", separate_plots=True):
    # 日付列をdatetime型に変換
    ...
    if unit == "day":
        ...
        # y_limit = -30000  # 日ごとのy軸範囲
        y_limit = 1000000
    elif unit == "month":
        grouper = pd.Grouper(key="日付", freq="M")
        # y_limit = -200000  # 月ごとのy軸範囲
        y_limit = 300000
    else:
        raise ValueError("Invalid unit. Use 'day' or 'month'.")
    ...
    if not separate_plots:
        title = f"売り上げ合計推移 ({unit}単位)"
        plt.title(title)
        plt.grid()
        plt.show()


def plot_sales_by_store_category(df, unit="day", separate_plots=True):
    # 日付列をdatetime型に変換
    df["日付"] = pd.to_datetime(df["日付"])
    ...
    for store_id in unique_stores:
        ...
        sales_by_store_category = (
            store_data.groupby(["店舗ID", "商品カテゴリ名", grouper])["売上"]
            .sum()
            .reset_index()
        )
        ...
        plt.xlabel(unit.capitalize())
        plt.ylabel("売り上げ")
        plt.legend(title="商品カテゴリ名", loc="upper left", bbox_to_anchor=(1, 1))
